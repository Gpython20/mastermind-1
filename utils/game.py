...
class Game():
    ...
    def update_game_data(self, gameid, plays, status):
        db_client = DBClient()
        db_client.connect()
        db_client.update_game_data(gameid, plays, status)
        db_client.close()
        return
    ...
    def save_play(self, gameid, play, guess, key_pegs):
        db_client = DBClient()
        db_client.connect()
        db_client.insert_play(gameid, play, guess, key_pegs)
        db_client.close()
        return
    ...
    def guess(self, gameid, guess):
        ...
        self.update_game_data(gameid, new_play, new_status)
        self.save_play(gameid, new_play, json.dumps(guess), json.dumps(key_pegs) )

        return {'key_pegs':key_pegs, 'plays': new_play, 'status': new_status}
